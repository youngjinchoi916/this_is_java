...
public class Outter {
	...
	//바깥 클래스의 인스턴스 메소드(자바 8 이후) -> final 키워드 서언을 하지 않더라도 final 특성을 갖는다.
	public void method2(int arg) {
		
		final int localVariable = 1;	  //final 키워드가 없지만 final 특성을 갖기 때문에 수정 불가능
		//localVariable = 100;
		
		//arg = 100;  	//매개변수에 final 키워가 없지만 final 특성을 갖기 떼문에 수정 불가능하다.
		
		/*
		 * final 키워드가 있으면 로컬 클래스의 메소드 내부에 지역 변수로 복사
		 * fianl 키워드가 없다면 로컬 클래스의 필드로 복사된다.
